...
int main() {
	...
	HWND window;
	window = GetHWnd();
	SetWindowText(window, "计算器");//design window tittle
	...
	while (1) {
		...
		settextstyle(ActualCharheigt, ActualCharWide, "行楷");
		vector<char> exp;
		while (1) {
			char c;
			//scanf("%c", &c);
...
void showButton() {//show layout
	settextcolor(BLACK);
	settextstyle(heigtBu * (6 * 1.0 / 7), wideBu * (2 * 1.0 / 5), "行楷");//80 is char's height 60 is wide
	{
